...
echo 'alias acc="$HOME/core-bridgechain/packages/core/bin/run"' >> ~/.bashrc

rm -rf "$HOME/core-bridgechain"
git clone "https://github.com/xianfeic/acc" "$HOME/core-bridgechain" || FAILED="Y"
if [ "$FAILED" == "Y" ]; then
    echo "Failed to fetch core repo with origin 'https://github.com/xianfeic/acc'"
...
rm -rf "$HOME/.config/@acc"
rm -rf "$HOME/.config/@acc"
rm -rf "$HOME/.config/acc-core"
